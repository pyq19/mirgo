...
func encodeTradeItem(msgObj *SM_TradeItem) (data interface{}, err error) {
	var bytes []byte
...
func encodeSplitItem(msgObj *SM_SplitItem) (data interface{}, err error) {
	var bytes []byte
	msg := msgObj
	writer := &BytesWrapper{Bytes: &bytes}
	if msg.Item != nil {
...
func encodePlayerInspect(msgObj *SM_PlayerInspect) (data interface{}, err error) {
	var bytes []byte
	pi := msgObj
	writer := &BytesWrapper{Bytes: &bytes}
...
func encodeNPCResponse(msgObj *SM_NPCResponse) (data interface{}, err error) {
	var bytes []byte
	res := msgObj
	writer := &BytesWrapper{Bytes: &bytes}
	count := len(res.Page)
...
func encodeObjectNPC(msgObj *SM_ObjectNPC) (data interface{}, err error) {
	var bytes []byte
	on := msgObj
	writer := &BytesWrapper{Bytes: &bytes}
...
func encodeObjectPlayer(msgObj *SM_ObjectPlayer) (data interface{}, err error) {
	var bytes []byte
	op := msgObj
...
func encodeUserInformation(msgObj *SM_UserInformation) (data interface{}, err error) {
	//return encode(msgObj)
	var bytes []byte
	ui := msgObj
	writer := &BytesWrapper{Bytes: &bytes}
...
// Decode 将字节数组转换为数据
func decodePlayerInspect(data interface{}, msgObj *SM_PlayerInspect) error {
	pi := msgObj
	bytes := data.([]byte)
	...
	pi.Equipment = make([]*UserItem, count)
	...
	pi.Class = MirClass(reader.ReadByte())
	pi.Gender = MirGender(reader.ReadByte())
...
func decodeUserInformation(data interface{}, msgObj *SM_UserInformation) error {
	ui := msgObj
	bytes := data.([]byte)
	reader := &BytesWrapper{Bytes: &bytes}
	...
	x := reader.ReadInt32()
	y := reader.ReadInt32()
	ui.Location = Point{X: uint32(x), Y: uint32(y)}
	...
	ui.LevelEffect = LevelEffects(reader.ReadUInt8())

	// Inventory
	if reader.ReadBoolean() {
		count := reader.ReadInt32()
		ui.Inventory = make([]*UserItem, count)
		for i := 0; i < int(count); i++ {
			if reader.ReadBoolean() {
				last := reader.Last()
				// FIXME
	...
	if reader.ReadBoolean() {
		count := reader.ReadInt32()
		ui.Equipment = make([]*UserItem, count)
		for i := 0; i < int(count); i++ {
			if reader.ReadBoolean() {
				last := reader.Last()
				// FIXME
	...
	if reader.ReadBoolean() {
		count := reader.ReadInt32()
		ui.QuestInventory = make([]*UserItem, count)
		for i := 0; i < int(count); i++ {
			if reader.ReadBoolean() {
				last := reader.Last()
				// FIXME
	...
	count := reader.ReadInt32()
	clientMagics := make([]*ClientMagic, 0)
	for i := 0; i < int(count); i++ {
		last := reader.Last()
		magic := new(ClientMagic)
...
func decodeObjectPlayer(data interface{}, msgObj *SM_ObjectPlayer) error {
	op := msgObj
	bytes := data.([]byte)
	reader := &BytesWrapper{Bytes: &bytes}
	...
	op.Class = MirClass(reader.ReadByte())
	op.Gender = MirGender(reader.ReadByte())
	op.Level = reader.ReadUInt16()
	op.Location = Point{X: uint32(reader.ReadInt32()), Y: uint32(reader.ReadInt32())}
	op.Direction = MirDirection(reader.ReadByte())
	...
	op.Poison = PoisonType(reader.ReadUInt16())
	op.Dead = reader.ReadBoolean()
	op.Hidden = reader.ReadBoolean()
	op.Effect = SpellEffect(reader.ReadByte())
	...
	bc := reader.ReadInt32()
	op.Buffs = make([]BuffType, bc)
	for i := 0; i < int(bc); i++ {
		op.Buffs[i] = BuffType(reader.ReadByte())
	}

	op.LevelEffects = LevelEffects(reader.ReadByte())
	return nil
}

func decodeObjectNPC(data interface{}, msgObj *SM_ObjectNPC) error {
	on := msgObj
	bytes := data.([]byte)
	reader := &BytesWrapper{Bytes: &bytes}
	...
	on.Location = Point{X: uint32(reader.ReadInt32()), Y: uint32(reader.ReadInt32())}
	on.Direction = MirDirection(reader.ReadByte())
...
func decodeNPCResponse(data interface{}, msgObj *SM_NPCResponse) error {
	res := msgObj
	bytes := data.([]byte)
	reader := &BytesWrapper{Bytes: &bytes}
